...
The adapter was implemented against the official DeepSeek Harness
`dsh-v0.1.0-rc.8` tag at commit
`141eb6fef83422698aef7a981029e843e8161534` and its current
`SESSION_FORMAT_VERSION = 0` JSONL persistence implementation. The input is
the logical JSONL returned by DSH `SessionPersistence.readRaw()`; physical
Zstandard decoding remains caller-owned.

Sanitized fixtures retain only the upstream event structure. They cover native
message IDs, event sequences and timestamps, provider/model routing, ordered
reasoning and tool-call blocks, raw tool arguments, successful and failed tool
results, packed stream-chunk rows, and model-only compaction replacement
events. The tests prove that `isError` maps to `ok`, the assembled assistant
message is not duplicated by the later `tool/call` lifecycle event, and
canonical source identities use DSH message IDs while ordering uses event
`seq`. No private transcript was used.

The normalized v1 schema has one model metadata string, so DSH's two-part
route is represented as `provider/model` rather than dropping the provider. A
future session format version is rejected until requalified because DSH
currently promises no pre-release on-disk compatibility.
